...
paster serve --reload development.ini</pre>
    ...
    <ul>
        <li>Pour ouvrir l'interface utilisateur, il vous suffit d'ouvrir votre navigateur internet et d'aller à l'adresse du lien suivant : <kbd>http://localhost:2016</kbd>
        </li>
    </ul>
    <hr>
...
##Installation Windows
    ...
    <ul>
        <li>Installer <a href="https://www.python.org/download/releases/2.7.6/">Python 2.7.6</a>
        </li>
...
##Installation Heroku
    ...
    <ul>
        ...
        <li>
            Une fois installé, vous aurez accès à la commande heroku de votre shell. Connectez-vous en utilisant l'adresse e-mail et mot 
            de passe que vous avez utilisé lors de la création de votre compte Heroku :
            <pre>heroku login</pre>
        </li>
        <li>Une fois installé, il faut se rendre dans son dossier "openfisca-web-site" et effectuer les commandes suivante afin de lancer le serveur sur Heroku :
            <pre>heroku create --region eu --buildpack https://github.com/jdesboeufs/heroku-buildpack-openfisca
git push heroku master
heroku open</pre>
        </li>
    </ul>
    <div class="alert alert-danger"><strong>Attention :</strong> pour lancer vos tests sur Heroku, dans vos fichiers test, vous devez remplacer les liens de simulation et de législation par le lien de la page qui s'est ouvert quand vous avez fait "heroku open" :
        <ul>
            <li>http://api.openfisca.fr/api/1/simulate  --&gt;  http://<kbd>[lien heroku]</kbd>/api/1/simulate</li>
            <li>http://api.openfisca.fr/api/1/default-legislation  --&gt;  http://<kbd>[lien heroku]</kbd>/api/1/default-legislation</li>
        </ul>
    </div>
    <hr>
